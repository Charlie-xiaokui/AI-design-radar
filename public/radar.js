// Generated from public/radar.ts.
...
const heroMediaBySignalId = new Map();
...
function parseHomepageCandidate(value) {
...
function homepageCandidates() {
    return allSignals.filter((signal)=>signal.status === "approved" && signal.homepage_candidate === true && signal.homepage_score >= 3).sort((a, b)=>Number(hasMedia(b)) - Number(hasMedia(a)) || b.homepage_score - a.homepage_score || timestamp(b) - timestamp(a));
}
...
function hasMedia(signal) {
    return safeArray(signal.media_urls).length > 0;
}
function heroMedia(signal) {
    return heroMediaBySignalId.get(signal.id) ?? {
        url: "",
        kind: "none",
        hero_media_kind: "unknown",
        rejected: []
    };
}
function hasUsableHeroMedia(signal) {
    return heroMedia(signal).kind !== "none";
}
function mediaKindFor(url, type = "") {
...
function mediaContext(signal, url) {
    return [
        url,
        urlBasename(url),
        signal.title,
...
function classifyHeroMedia(signal, url, kind) {
    ...
    if (productUiPattern.test(context)) {
        return /(workflow|agent|builder|canvas|editor|workspace|dashboard|ide|terminal|diff|chat|panel)/i.test(context) ? "workflow_visual" : "product_ui";
    }
    if (kind === "video" || kind === "gif") return "workflow_visual";
    return "unknown";
}
function rejectUrlBeforeLoad(signal, url, kind) {
...
async function inspectMediaCandidate(signal, url, kind, priority) {
    ...
    if (![
        ...
        "workflow_visual",
        "concept_mockup"
    ].includes(hero_media_kind)) return {
        url,
        reason: `未识别为产品界面图（${heroMediaKindLabels[hero_media_kind]}）`
    };
    ...
    try {
        ...
        if (width <= 2 || height <= 2) return {
            url,
            reason: `疑似 tracking pixel 或空白图 (${width}x${height})`
        };
        if (width < 360 || height < 180 || width * height < 120000) return {
            url,
            reason: `尺寸过小，不适合首页产品图 (${width}x${height})`
        };
        return {
            url,
...
async function selectHeroMedia(signal) {
    const urls = safeArray(signal.media_urls);
    const types = safeArray(signal.media_types);
    ...
    const accepted = inspected.filter((item)=>"kind" in item && item.kind !== "none");
    const rejected = inspected.filter((item)=>"reason" in item);
    accepted.sort((a, b)=>b.priority - a.priority || b.area - a.area);
    const selected = accepted[0];
    return selected ? {
        url: selected.url,
        kind: selected.kind,
        hero_media_kind: selected.hero_media_kind,
...
function mediaMarkup(signal, large = false) {
    ...
    return `<div class="card-media ${large ? "large" : ""} empty"><div><strong>暂无可用产品界面图</strong><small>${safeArray(signal.media_urls).length ? "已发现媒体，但未识别到可用于首页的产品界面图。" : `等待 ${escapeHtml(categoryLabels[signal.homepage_category] ?? "设计信号")} 的产品截图`}</small></div></div>`;
}
...
function descriptionFor(signal, limit = 180) {
    const text = signal.description || signal.raw_text || "";
    return text.length > limit ? `${text.slice(0, limit).trim()}...` : text;
}
function reasonList(signal) {
    const reasons = safeArray(signal.homepage_reasons).slice(0, 3);
...
function renderMetrics() {
    ...
    const counts = Object.fromEntries(filterCategories.map((category)=>[
            category,
            candidates.filter((signal)=>signal.homepage_category === category).length
        ]));
    $("#radarMetrics").innerHTML = [
        ...
        [
            withUsableHero.length,
            "可用界面图"
        ],
        [
            counts.agent_experience ?? 0,
            "Agent 表达方式"
        ],
        [
            counts.concept ?? 0,
...
function renderGrid() {
    ...
    $("#radarGrid").innerHTML = visible.length ? visible.map(cardMarkup).join("") : `<div class="empty-radar"><strong>这个分类暂时没有卡片。</strong><span>在信源后台审核首页候选后，这里会自动出现。</span></div>`;
}
function render() {
    renderMetrics();
    renderFilters();
...
function detailMarkup(signal) {
    const media = heroMedia(signal);
    const rejected = media.rejected.length ? `<ul>${media.rejected.map((item)=>`<li><span>${escapeHtml(item.reason)}</span><code>${escapeHtml(item.url)}</code></li>`).join("")}</ul>` : `<p>没有被拒绝的媒体。</p>`;
    ...
    <div class="detail-copy">
      ...
      <section class="media-debug">
        <h3>媒体调试信息</h3>
        <dl>
          <div><dt>媒体 URL 数量</dt><dd>${safeArray(signal.media_urls).length}</dd></div>
        ...
        ${safeArray(signal.media_urls).length && !media.url ? "<p>已发现媒体，但未识别到可用于首页的产品界面图。</p>" : ""}
        <h4>被拒绝的媒体</h4>
        ${rejected}
      </section>
...
function openDetail(signalId) {
    ...
    $("#radarDetailDialog").showModal();
}
async function load() {
    const response = await fetch("/api/raw-signals");
    if (!response.ok) throw new Error("无法加载 Raw Signals");
    allSignals = safeArray(await response.json()).map(normalizeSignal);
    const candidates = homepageCandidates();
    const selections = await Promise.all(candidates.map(async (signal)=>[
            signal.id,
            await selectHeroMedia(signal)
        ]));
